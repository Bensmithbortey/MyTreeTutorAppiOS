    ...
    var body: some View {
        VStack {
            VStack(alignment: .leading, spacing: 20) {
                ForEach(contentText) { contentItem in
                    if let image = contentItem.data as? ImageContent {
                        Image(image.imageName)
                            .frame(width: 300, height: 300)
                            .clipped()
                    }
                    if let item = contentItem.data as? TextualContent {
